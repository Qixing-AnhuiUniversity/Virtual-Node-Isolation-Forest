...
class iTree(object):
    ...
    def make_tree(self,X,e,l):
        self.e = e
        if e >= l or (len(X)<= 1 and e>3) :
            left = None
            right = None
            self.exnodes += 1
            return Node(X, self.n, self.p, e, left, right, node_type = 'exNode' )
        elif (len(X)<= 1 and e<= 3) :
            left = None
            right = None
            self.exnodes += 1
            return Node(X, self.n, self.p, e, left, right, node_type = 'vrNode' )
        else:
            mins = X.min(axis=0)
            maxs = X.max(axis=0)
    ...
    def find_path(self,T):
        ...
        if T.ntype == 'exNode':
            if T.size <= 1: return self.e
                ...
                return self.e
        elif T.ntype == 'vrNode' and self.path_list[-1] == 'L':
            #print("vrNodeL")
            b = T.n
            ii = 0
            #if T.size <= 1: return self.e
            ...
            #print("b = " + str(b))
            #ff = np.random.uniform([-3,-3,-3],[0,0,0])
            #ff = np.random.uniform(-3*T.n,[0,0,0])
            ff = -2*T.n
            d = ff+2*(ff)
            while (self.x-ff).dot(b) <= 0:
                #ii = ii + 1
                ff = d
                d = ff+2*(ff)
                g = g + 3
            ...
            self.e = self.e - g
            return self.e
        elif T.ntype == 'vrNode' and self.path_list[-1] == 'R':
            #print("vrNodeR")
            b = T.n
            ii = 0
            #if T.size <= 1: return self.e
            ...
            #print("b = " + str(b))
            #ff = np.random.uniform([0,0,0],[3,3,3])
            #ff = np.random.uniform([0,0,0],3*T.n)
            ff = 2*T.n
            d = ff+2*(ff)
            while (self.x-ff).dot(b) > 0:
                #ii = ii + 1
